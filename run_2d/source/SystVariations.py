...
class SystVariations:
    ...
    def CompareSyst(self, Summarize, IdSF, IsoSF, RecoSF, TrigSF, Recoil, Calib, channel, Indice):
        ...
        for i in range(0, len(hists)):
            ...
            hists[i].GetYaxis().SetRangeUser(0.,2.5)
            hists[i].SetStats(0)
            hists[i].SetTitle("")
            if(Colorcompteur == 5):
                Colorcompteur = 28
	...
	astyle.SetAtlasStyle()
        ...
        legend1 = makeLegend(hists1,0.35, 0.45,0.5,0.7)
        legend2 = makeLegend(hists2,0.55, 0.45,0.7,0.7)
        legend1.Draw("same")
        legend2.Draw("same")
	...
	astyle.SetAtlasStyle()
        ...
        for hist in hists:
            hist.Draw("same")
        astyle.ATLASLabel(0.4, 0.82, "Internal")
        utils.DrawText(0.4, 0.76, Indice)

        legend1 = makeLegend(hists1,0.3, 0.35,0.5,0.6)
        legend2 = makeLegend(hists2,0.6, 0.35,0.8,0.6)
        legend1.Draw("same")
        legend2.Draw("same")
        c1.Update()
	...
	astyle.SetAtlasStyle()
        ...
        legend1 = makeLegend(hists1,0.3, 0.45,0.5,0.7)
        legend2 = makeLegend(hists2,0.6, 0.45,0.8,0.7)
        legend1.Draw("same")
        legend2.Draw("same")
	...
	hists2 = []
        ...
        while compteur < NumberOfIterationMaximal:
            ...
            hist.SetMarkerColor(Colorcompteur)
            hist.GetYaxis().SetRangeUser(0,0.9)
            hist.GetXaxis().SetTitle("p^{l}_{T} [GeV]")
        ...
        legend1 = makeLegend(hists1,0.4, 0.45,0.55,0.7)
        legend2 = makeLegend(hists2,0.6, 0.45,0.75,0.7)
        legend1.Draw("same")
        legend2.Draw("same")
    ...
    def CompareSystTrig(self, inputFile, NumberOfIterationMinimal, NumberOfIterationMaximal, channel, Indice):
        ...
        while compteur < NumberOfIterationMaximal:
            ...
            hist.SetMarkerColor(Colorcompteur)
            hist.GetYaxis().SetRangeUser(0.05,0.4)
            hist.GetXaxis().SetTitle("p^{l}_{T} [GeV]")
 	...
 	astyle.SetAtlasStyle()
        ...
        legend1 = makeLegend(hists1,0.3, 0.45,0.55,0.7)
        legend2 = makeLegend(hists2,0.6, 0.45,0.85,0.7)
        legend1.Draw("same")
        legend2.Draw("same")
