...
class CrossSection:
    ...
    def GetptwTables(self, Summarize_minusenu5, Bias, TrigSF, RecoSF, IsoSF, IdSF, Calib, Recoil, Energy, Indice, Name, Lum, Niter):
        ...
        latexFile = open("Output/LatexTableau/pTW_table_"+Name+".tex","w+")
        latexFile.write("\\documentclass[12pt]{article} \n")
        latexFile.write("\\usepackage{amsmath}  \n")
        latexFile.write("\\usepackage{graphicx} \n")
        latexFile.write("\\usepackage{hyperref} \n")
        ...
        latexFile.write("\\multicolumn{1}{|c|}{  range } & $p^{T}_{W}$ [GeV]     & Stat uncertainty     & Unfolding bias     & Syst uncertainty        \\\ \\hline \\hline \n")

        i = 0
        while i < 1+(HUnfolded.GetNbinsX())/2:
            diffXs   = HUnfolded.GetBinContent(i+1)/Acceptance.GetBinContent(i+1)
	...
	# define the Diff Xs:
        ...
        latexFile.write("& \\multicolumn{4}{c|}{%s}  \\\ \\hline \\hline \n" %(Indice))
        latexFile.write("\\multicolumn{1}{|c|}{  range } & $d\sigma$/$dp^{T}_{W}$ [GeV]     & Stat uncertainty     & Unfolding bias     & Syst uncertainty        \\\ \\hline \\hline \n")
	...
	DXs.GetXaxis().SetRangeUser(0,60)
	DXs.SetStats(0)
	DXs.SetTitle("")
	DXs.GetYaxis().SetTitle("d#sigma / dp_{w}^{T} ")
	DXs.GetXaxis().SetTitle("p_{w}^{T} [GeV]")
	...
	hists = []
        ...
        Ratio1.GetXaxis().SetTitle("p_{w}^{T} [GeV]")
	...
	Ratio1.GetXaxis().SetRangeUser(0,60)
	Ratio1.SetLineWidth(2)
        Ratio1.SetLineColor(2)
        Ratio1.Draw("same")

        line1 = ROOT.TLine(0,0.95,64,0.95)
        line2 = ROOT.TLine(0,1.05,64,1.05)
	...
	Xs = ( HUnfolded.Integral()/Lum ) * ( truth.Integral() / (mig_mat.ProjectionY()).Integral() )
        ...
        DXs.SetStats(0)
        DXs.SetTitle("")
        DXs.GetYaxis().SetTitle("1/d#sigma . d#sigma / dp_{w}^{T} ")
        DXs.GetXaxis().SetTitle("p_{w}^{T} [GeV]")
        DXs.GetYaxis().SetTitleOffset(0.9)
        DXs.GetXaxis().SetTitleOffset(1.4)
        DXs.GetYaxis().SetTitleSize(0.07)
