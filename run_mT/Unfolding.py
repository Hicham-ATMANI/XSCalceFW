...
if( cfg['Channel'] == "Wminusenu" and cfg['Energy'] == 5):
	...
	Input_SFid    = TFile( cfg['Wminusenu5_IdSF'] );     Input_SFiso  = TFile( cfg['Wminusenu5_IsoSF'] );   Input_SFreco    = TFile( cfg['Wminusenu5_recoSF'] );  Input_SFTrig = TFile( cfg['Wminusenu5_TrigSF'] ); 
        fInput_MC_BS  = TFile( cfg['Wminusenu5_BSMC'] );     fInput_TemP  = TFile( cfg['Wminusenu5_Tem'] );
...
mig_hist   = MigrationMatrix( fInput_MC, cfg['Channel'], cfg['Energy'], cfg['Variable'])
...
data_hist  = RecoDistribution( fInput_Data, cfg['Channel'], cfg['Variable']) 
...
recoCorrected = CorrectData( reco_hist, reco_hist, Background_Total, Efficiency_hist)

# read MC Templates:
indice       = ["","_eta1", "_eta2", "_eta3", "_eta4"]
MCTemplates  = ReadMCTemplate( fInput_TemP, cfg['Channel'], cfg['Variable'], indice[0])
...
ToysOfMC   = GetToysofData(fInput_MC_BS, cfg['Channel'], cfg['Variable'], cfg['Energy'])


for i in range(0, len(ToysOfData) ):
    ToysOfData[i].Add(data_hist)
    for j in range(0, data_hist.GetNbinsX() ):
	ToysOfData[i].SetBinError(j+1, data_hist.GetBinError(j+1))
    ToysOfData[i] =  CorrectData( ToysOfData[i], reco_hist, Background_Total, Efficiency_hist)

for i in range(0, len(ToysOfMC) ):
    ToysOfMC[i].Add(reco_hist)
    for j in range(0, reco_hist.GetNbinsX() ):
        ToysOfMC[i].SetBinError(j+1, reco_hist.GetBinError(j+1))
    ToysOfMC[i] =  CorrectData( ToysOfMC[i], reco_hist, Background_Total, Efficiency_hist)
...
for i in range(1, 1+cfg['Niterations']):
    print(" Iteration %d"%(i))
    unfoldMC      = RooUnfoldBayes (responseM, recoCorrected, i)
    MCNominal     = unfoldMC.Hreco()
    UnfoldMCToys  = GetUnfoldToys(responseM, ToysOfMC, i)
    CovarianceMC  = GetCovarianceMatrix(MCNominal, UnfoldMCToys, VarMin, VarMax, cfg['Variable'], mig_hist)
    CovarianceMCMatrix.append(CovarianceMC)
...
for i in range(0, len(ToysOfMC) ):
    ToysOfMC[i].Write("Toys_MC"+str(i+1))
for i in range(0, len(CovarianceMatrix) ):
    CovarianceMatrix[i].Write("CovarianceMatrix_Iter"+str(i+1))       
for i in range(0, len(CovarianceMCMatrix) ):
    CovarianceMCMatrix[i].Write("CovarianceMatrix_MC_Iter"+str(i+1))
'''
print("Fin")
