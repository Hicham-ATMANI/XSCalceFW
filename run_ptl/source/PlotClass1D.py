...
import ROOT
import ROOT as root
from   ROOT import gROOT, TCanvas, TFile, THStack, TH1F, TPad, TLine, TAttFill, TF1, TStyle
...
class Plot1D:
	...
	dataO   = inputFile.Get("dataCorrected")
	data	= reco.Clone("data")

	for i in range(0, data.GetNbinsX()):
	    data.SetBinContent(i+1, dataO.GetBinContent(i+1))
	    data.SetBinError(i+1, dataO.GetBinError(i+1))
	...
	stylee   = 1
	    ...
	    hist.SetMarkerSize(1)
            ...
            hist.GetXaxis().SetRangeUser(50,110)
	    ...
	    compteur +=1
        astyle.ATLASLabel(0.65, 0.82, "Internal")
        utils.DrawText(0.65, 0.76, Indice)
	...
	for i in range(0, len(hists)/2):
        ...
        legend1 = makeLegend(hists1,0.3, 0.45,0.5,0.75)
        legend2 = makeLegend(hists2,0.6, 0.45,0.8,0.75)
        legend1.Draw("same")
        legend2.Draw("same")
        c1.Update()
    ...
    def CompareStatError(self, inputFile, NumberOfIterationMinimal, NumberOfIterationMaximal, channel, Indice):
        ...
        while compteur < NumberOfIterationMaximal:
            ...
            StatErrorHist.GetYaxis().SetRangeUser(0,3.5)
