...
'''
def OptimisationStudy( inputFile, Bias, TrigSF, RecoSF, IsoSF, IdSF, Calib, Recoil, channel):
        ...
        for compteur in range(0, 9):
            ...
            for i in range(1, 1 + CovMatrix.GetNbinsX()):
                for j in range(1, 1+CovMatrix.GetNbinsX()):
                    if(channel.find("enu")  != -1):
                        CovMatrix.SetBinContent(i, j, BiasCovMatrix.GetBinContent(i,j) + TrigCovMatrix.GetBinContent(i,j) + RecoCovMatrix.GetBinContent(i,j) + IsoCovMatrix.GetBinContent(i,j) +
                                                    IDCovMatrix.GetBinContent(i,j) + CalibCovMatrix.GetBinContent(i,j) + RecoilCovMatrix.GetBinContent(i,j) + StatCovMatrix.GetBinContent(i,j) )
                    if(channel.find("munu")  != -1):
                        CovMatrix.SetBinContent(i, j, BiasCovMatrix.GetBinContent(i,j) + TrigCovMatrix.GetBinContent(i,j) + RecoCovMatrix.GetBinContent(i,j) + IsoCovMatrix.GetBinContent(i,j) +
                                                    RecoilCovMatrix.GetBinContent(i,j) + StatCovMatrix.GetBinContent(i,j) )
...
class OpitmisationStudy1B:
		...
		if( binContent[i] != 0):
            ...
            pylab.plot(x, StatError,   marker='o', label =  '[' + str( int(data_Unfolded.GetBinLowEdge(Nbin)) ) + ',' + str( int(data_Unfolded.GetBinLowEdge(Nbin+1))) + ']')
            pylab.xlabel('Iterations');
            pylab.ylabel('Stat [%]');
            pylab.legend(loc='center right')
            text(, y, "ddddddddd", fontsize=12)
		...
		Error = Error + CovMatrix.GetBinContent(Nbin, Nbin)
            ...
            x=np.linspace(1,IterMax,IterMax)
            pylab.plot(x, StatError,   marker='o', label =  '[' + str( int(data_Unfolded.GetBinLowEdge(Nbin)) ) + '-' + str( int(data_Unfolded.GetBinLowEdge(Nbin+1))) + '] GeV')
            pylab.xlabel('Iterations');
